...
func Router(cli client.Client, clientset kubernetes.Interface) *gin.Engine {
	...
	router.POST("/api/auth", AuthHandler(rds, clientset, logger))
...
func InitClients() (cli client.Client, clientset kubernetes.Interface) {
	...
	redislibsent.AddToScheme(scheme)
...
func GetRepl(cli client.Client, ctx context.Context, namespace string, name string) (*redislibrepl.RedisReplication, error) {
	var repl redislibrepl.RedisReplication
